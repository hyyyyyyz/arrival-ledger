...
| 场景 | 入口 | 是否需要密码 | 当前状态 |
|---|---|---:|---|
| 仓库局域网 | 微信 H5，访问 http://192.168.1.5:8766 | 否 | 正在从 .4 迁移到 .5，待真机完整验收 |
...
- 以上 Docker、到货管家容器、健康检查和免登录入口目前仍在旧机 .4 正常运行；
- 目标机 .5 当前尚未安装 Docker，也尚未部署到货管家；
- 目标机 8766 端口空闲，后端容器端口仍不直接暴露，由前端 Nginx 负责网关；
...
- 尚未完成 Windows 同步端；
- 尚未完成采购订单/CSV 导入模块；
- 当前目录尚未建立 Git 回滚基线，进入大改前应先初始化版本库或建立外部备份。

### 8.4 服务器迁移：192.168.1.4 → 192.168.1.5

目标机 .5 已有 cash-save/Caddy 使用 80、443 和 127.0.0.1:8000；arrival-ledger 继续使用 8766，不修改 Caddy 和 cash-save。目标机当前未安装 Docker，安装和启动需交互式 sudo。
...
4. 在 .5 安装 Docker/Compose，代码放 /home/jackson/arrival-ledger；
5. 还原数据到兼容路径 /home/jackson/arrival-manager-data 并恢复 UID/GID 10001 权限；
6. 在 .5 构建启动，验证健康、页面、历史照片、上传、重启和备份；
7. 手机确认 http://192.168.1.5:8766 可用后，停止 .4 的 arrival-manager Compose 栈；
8. 保存最终离机备份后，仅清理 .4 上本项目的容器、网络、镜像、代码目录和数据目录；
...
### 阶段 P-1：原型与局域网部署（已完成，待真机验收）

- [x] FastAPI/SQLite/H5/Docker 原型；
- [x] 拍照、压缩、条码识别、上传、哈希、幂等和重复提示；
- [x] IndexedDB 离线队列；
- [x] 192.168.1.4:8766 旧机局域网部署；
- [ ] 完成迁移并在 192.168.1.5:8766 验收；
- [ ] 微信真机拍摄 1 件；
- [ ] 连续 30 件、断网重传、多个条码验收；
- [ ] 交互式检查容器日志、备份和磁盘告警。
