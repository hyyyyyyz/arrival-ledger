...
export function cursorFileName(stateDir: string, platform: Platform): string {
  return join(stateDir, `cursor-${platform}.json`);
}
...
export function loadCursor(stateDir: string, platform: Platform): CursorState | null {
  try {
    const parsed = JSON.parse(
      readFileSync(cursorFileName(stateDir, platform), "utf8"),
    ) as Record<string, unknown>;
    if (
      !isPlatform(String(parsed["platform"])) ||
      parsed["platform"] !== platform ||
      typeof parsed["account_key"] !== "string" ||
...
export function saveCursor(stateDir: string, cursor: CursorState): void {
  ...
  const target = cursorFileName(stateDir, cursor.platform);
...
export function updateCursor(
  ...
  const current = loadCursor(stateDir, platform) ?? emptyCursor(platform, accountKey);
  const next: CursorState = { ...current, ...patch, updated_at: new Date().toISOString() };
  saveCursor(stateDir, next);
  return next;
}

export function removeCursor(stateDir: string, platform: Platform): void {
  rmSync(cursorFileName(stateDir, platform), { force: true });
}
