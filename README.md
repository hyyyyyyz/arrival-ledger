...
当前旧实例仍在 192.168.1.4:8766；目标实例使用 192.168.1.5:8766。目标机已有 Caddy/cash-save 占用 80、443 和 127.0.0.1:8000，本项目不得修改它们。迁移仅使用宿主机 8766。
...
只有目标机健康检查、真机上传、重启持久化和备份恢复均通过后，才可停止旧栈。清理旧机时只能删除本项目的旧 Compose 容器/网络/镜像以及旧应用/数据目录；不得使用全局 prune、不得执行 down -v，也不得影响 pharos:8848。删除前另存最终归档及 SHA-256。
