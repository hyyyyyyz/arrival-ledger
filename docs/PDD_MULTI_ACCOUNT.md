# 拼多多多账号人工同步（桌面端或服务器可见浏览器）

本文说明当前已经实现、可以验收的拼多多多账号方案。可由一台 **Mac/Windows 同步电脑**运行可见
Playwright 浏览器，也可由 Linux 服务器运行 Xvfb/noVNC 中的可见浏览器；服务器方式见
[`PDD_SERVER_AGENT.md`](PDD_SERVER_AGENT.md)。两种方式都只把结构化订单和脱敏状态发送给后端。

## 当前能力与边界

- 每个拼多多账号使用一个唯一、长期不变的 `account_key`；
- 每个账号使用一个独立、持久化、可见的 Playwright `profile_dir`；
- 登录、扫码、短信或人机验证由用户在同步端的可见窗口中完成；
- profile、Cookie、localStorage、sessionStorage、密码和验证码永远不进入后端 API 或业务数据库；
...
当前已经提供仅经 SSH 隧道访问的服务器 noVNC 监督式试验版，但**没有实现**公网管理页远程控制、自动
Cookie 注入、定时 commit 或无人值守验证码处理。不要把桌面端 profile/Cookie 复制到服务器；服务器
Profile 必须由对应账号在服务器官方登录页重新生成。
...
手机号或用户名，不应包含敏感信息。网页登记不会创建登录态；下一步仍需在实际运行浏览器的同步端配置
同一个键。
...
| 需登录 | 在实际同步端运行该账号的 `login-check` 并人工登录 |
| 需要验证 | 人工处理平台验证；程序不会代做或重试 |
| 页面变化 | 适配器无法可靠读取，停止 commit，先修复并重新 dry-run |
| 网络错误 | 检查同步端网络和服务器地址，避免高频重开平台页面 |
| 本次未执行 | 本次因配置、冷却或本机互斥保护未进入有效读取；查看提示后再处理，不代表账号被停用 |
| 未同步 | 账号已登记，但同步端尚未上报成功状态 |

网页的“刷新状态”只重新读取服务器已有状态，不会远程启动同步端或浏览器。

## 9. 当前运行方式

当前只支持人工命令。Mac/Windows 可直接显示浏览器；Linux 服务器可使用已交付的 Xvfb/noVNC Agent，
但 noVNC 只能经 SSH 隧道访问。定时 dry-run/commit、失败通知和网页远程执行均未交付。即使未来增加
定时任务，也必须先完成本文件的真机验收，保持账号串行、登录人工处理，并保留 dry-run → 人工确认 →
commit 的安全边界。
